...
inherit linux-info flag-o-matic systemd udev user
...
src_install() {
	...
	fowners root:apcupsd /sbin/apcupsd || die
	fperms 750 /sbin/apcupsd || die
	dodir /var/log/apcupsd || die
	fowners apcupsd:apcupsd /var/log/apcupsd || die
	fperms 0750 /var/log/apcupsd || die
	fperms 0750 /etc/apcupsd || die
	fowners -R root:apcupsd /etc/apcupsd || die
	fperms 0640 /etc/apcupsd/apcupsd.conf || die

	dodir /usr/libexec/apcupsd
	mv "${D}"/etc/apcupsd/{apccontrol,changeme,commfailure,commok,offbattery,onbattery,safe.apccontrol} "${D}"/usr/libexec/apcupsd/ || die
	fperms 0750 /usr/libexec/apcupsd/{apccontrol,changeme,commfailure,commok,offbattery,onbattery,safe.apccontrol} || die
	fowners -R root:apcupsd /usr/libexec/apcupsd || die
	fperms 0750 /usr/libexec/apcupsd || die

	sed -i 's,^SCRIPTDIR.*,SCRIPTDIR /usr/libexec/apcupsd/,' "${D}"/etc/apcupsd/apcupsd.conf || die
	sed -i 's,^SCRIPTDIR=.*,SCRIPTDIR=/usr/libexec/apcupsd/,' "${D}"/usr/libexec/apcupsd/{apccontrol,safe.apccontrol} || die
