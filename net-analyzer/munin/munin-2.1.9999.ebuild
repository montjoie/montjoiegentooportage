...
src_configure() {
	...
	sed -i 's,/usr/local/etc/,/etc/,g' Build.PL
	sed -i 's,/usr/local/,/usr/,g' Build.PL
...
src_compile() {
	#emake -j1
	perl Build.PL --installpaths vendor || die
	if use doc; then
		emake -C doc html
	fi
#	sed -i 's,/usr/local/etc/,/etc/,g' _build/build_params
#	sed -i 's,/usr/local/,/usr/,g' _build/build_params
}
...
#	sed -i 's,/usr/local/etc/,/etc/,g' _build/build_params
#	sed -i 's,/usr/local/,/usr/,g' _build/build_params
	perl Build --installpaths vendor install --destdir=${D}
